...
| `_wsb_try_fetch_transcript(entry, feed_cfg, supadata_client, transcript_rate_limit_event)` | Phase-1 helper for the WebSub processor. Checks for a cached `transcript.txt`; if absent, calls `fetch_transcript()` and caches the result. Returns one of `"cached"`, `"success"`, `"no_captions"`, `"transient"`, `"permanent"`, `"livestream"`, or `"quota_exhausted"`. `"no_captions"` (Supadata answered: no captions) is capped at `_NO_CAPTIONS_MAX_ATTEMPTS`; `"transient"` (no answer — network/service error) gets the full `_TRANSCRIPT_MAX_ATTEMPTS` schedule. Never runs Gemini. |
