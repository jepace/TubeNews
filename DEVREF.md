# DEVREF.md — TubeNews Full Developer Reference
...
├── README.md
├── DEVREF.md                # This file (full developer reference)
...
| `_wsb_lease_healthy(channel_id, subs, callback_url)` | True when a recorded WebSub lease is worth keeping: present, pointing at the current `websub_callback_url`, and not due for renewal within 24 h (the same threshold the processor's renewal check uses, so startup and renewal agree). Daemon startup consults this, so a restart no longer re-subscribes every enabled channel and unsubscribes every disabled one unconditionally — that cost one hub round trip per channel, plus a verification callback per subscribe, to re-assert what was already true. A changed callback URL invalidates the record, since the hub would otherwise keep pushing to the old address. |
| `_wsb_hub_backoff_remaining()` / `_wsb_note_hub_throttle(retry_after, channel_id)` | The hub rate-limits clients and answers 429/503 with `Retry-After`. The throttle is recorded hub-wide (not per channel — the limit is on us as a client) and gates both subscribe and unsubscribe. A missing or unparseable header falls back to `_WSB_HUB_BACKOFF_FALLBACK` (120s); an absurd one is capped at `_WSB_HUB_BACKOFF_MAX` (1h). |
| `_wait_for_receiver(port, timeout=10)` | Blocks until the WebSub receiver accepts connections on *port*, so the hub has something to verify against before the first subscribe POST. Returns False on timeout after logging it; startup subscribes anyway and the processor retries failures. |
| `_read_subscriptions()` | Returns `state/subscriptions.json` as a dict; a missing, unparseable, or wrong-shaped file reads as `{}`. | The processor also re-attempts any enabled channel that holds no healthy lease and has no record at all — a subscribe POST that fails is never written to `subscriptions.json`, so the renewal check (which iterates that file) would never see it and the channel would silently receive no pushes until the next restart. Rate-limited per channel by `_WSB_RENEWAL_RETRY_COOLDOWN`.
...
| `_cleanup_stale_unverified_users()` | Scans `state/users/` and deletes any account where `email_verified` is `false` and more than `_UNVERIFIED_USER_MAX_AGE_HOURS` (72h) have passed since `email_verification_sent_at` (or `created_at` if that key is absent) — mainly registration spam/bots that never complete verification. Resending the verification email refreshes `email_verification_sent_at`, giving a real but slow-to-verify user a fresh window. Accounts with no `email_verified` key at all (pre-dating the feature) default to verified and are never touched, matching `User.is_verified` in web/app.py. Also drops the deleted account's entry from `state/users/index.json`. Returns the number of accounts deleted. Called once per 24 hours by `_wsb_processor_thread`, same cadence as `_recover_orphaned_videos`. |
...
| `_write_no_transcript_metadata(video_id, feed_dir, video_date, video_title, skip_reason)` | Creates the meeting directory if needed and writes `metadata.json` with `status = "no_transcript_available"`. Called from `process_video` (grace-window path, `_NO_CAPTIONS_GRACE_HOURS`), from the WebSub processor on retry exhaustion, and when a livestream never ends (`skip_reason="livestream_never_ended"`). |
| `_wsb_try_fetch_transcript(entry, feed_cfg, supadata_client, transcript_rate_limit_event)` | Phase-1 helper for the WebSub processor. Checks for a cached `transcript.txt`; if absent, calls `fetch_transcript()` and caches the result. Returns one of `"cached"`, `"success"`, `"no_captions"`, `"transient"`, `"permanent"`, `"livestream"`, or `"quota_exhausted"`. `"no_captions"` (Supadata answered: no captions) is capped at `_NO_CAPTIONS_MAX_ATTEMPTS`; `"transient"` (no answer — network/service error) gets the full `_TRANSCRIPT_MAX_ATTEMPTS` schedule. Never runs Gemini. | Phase 1 checks for a cached `transcript.txt` **before** consulting the credit budget, and on `"quota_exhausted"` the caller sets a `transcript_budget_spent` flag and keeps scanning rather than breaking — entries still needing a fetch stay queued untouched, while ones already cached continue to the Gemini phase.
...
`skip_reason` — present only when `status` is `"no_transcript_available"`. Values: `"no_captions"` (Supadata returned empty content or a generic error), `"members_only_or_restricted"` (HTTP 403 from Supadata — video is members-only or region-restricted), `"video_not_found"` (HTTP 404 from Supadata — video ID is invalid or deleted), `"livestream_never_ended"` (still broadcasting after `_LIVESTREAM_MAX_ATTEMPTS` checks). Key absent on all other status values and on older entries.
...
- `transcript_attempts`: Count of transcript fetch attempts that failed. New entries start at `0`. The cap depends on *why* it failed: `_NO_CAPTIONS_MAX_ATTEMPTS` (5) when Supadata answered "no captions", `_TRANSCRIPT_MAX_ATTEMPTS` (= `len(_TRANSCRIPT_RETRY_OFFSETS)`) for network/service errors. On exhaustion the video is written to disk as `no_transcript_available` and removed from the queue. Reset to `0` when a livestream entry is re-queued after the stream ends.
- `livestream_attempts`: Count of times the entry was re-queued because the stream was still broadcasting. Deliberately *not* reset by `_requeue_video` (unlike `transcript_attempts`), so a stream that never ends stops at `_LIVESTREAM_MAX_ATTEMPTS` (12) instead of being re-billed hourly forever.
...
| `websub_post_timeout` | No | Seconds to wait on a WebSub subscribe/unsubscribe POST (default `10`). Google's hub can be slow; raise this if subscribes time out. Startup attempts them serially, so a large value lengthens startup — the processor's periodic retry is the real safety net. |
...
| File | Covers |
|---|---|
| `tests/test_tubenews.py` | `slugify`, `parse_story_file` (including `topics`, `user_ids`, and HTML escaping in `body_html`), `_story_matches_focus`, `write_story_files` (including `**Users:**` output), `_collect_channel_focuses` (tuple return type, user-id merging), `discover_videos` (RSS parse, HTTP error, malformed XML, network retry, empty feed warning, no `is_live` field), `fetch_transcript` (success path, language mismatch, live-stream skip, quota exhaustion + event flag), the JSON extraction regex in `call_gemini_api`, `rebuild_feed`, `rebuild_aggregate_feed`, `build_user_feed_xml`, lock/unlock helpers, config resolution, `_recover_orphaned_videos` (queues dirs without metadata, skips dirs with metadata, skips already-queued, skips missing channel.json, returns count), `_cleanup_stale_unverified_users` (removes expired, keeps recent, keeps verified, keeps legacy no-key accounts, falls back to created_at, drops index entry, no-op on missing dir, skips corrupt json), `_iter_all_users` (yields user data, skips malformed), `_scan_stories_since` (returns recent stories, excludes old/unsubscribed), `_build_digest_html` (anchor links, HTML escaping), `_send_daily_digests` (skips no api key, skips no base_url, skips opted-out, sends to opted-in, filters old stories) |
| `tests/test_web.py` | `web/app.py` URL helpers (`_rss_url`, `_feed_url`), user preferences |
| `tests/test_webapp.py` | Flask routes: login guards, rate-limit (429 after 10 attempts), locked-account rejection, unverified-account rejection (explicit flash + no session), dashboard subscription save, admin guards, public token routes, lock-file detection, run-now trigger, channel browse YouTube link, admin runs channel links, `digest_email_enabled` preference save (checked and unchecked), registration honeypot (no account/email/ntfy on a filled field), new-user ntfy timing (silent on register, fires on verify), admin bulk-delete (removes selected, skips own account, requires admin/login, no-op on empty selection) |
...
- **Live stream handling:** The RSS feed includes in-progress livestreams with no indication they are live. When `fetch_transcript` is called for such a video, Supadata raises an exception whose message contains `"live streaming"`; `fetch_transcript` catches this and returns `None` (transient failure) with `livestream_error` set. The WebSub processor re-queues the entry roughly an hour out rather than retrying immediately. Each check costs one Supadata credit, so `livestream_attempts` caps the total at `_LIVESTREAM_MAX_ATTEMPTS`; past that the video is written off as `no_transcript_available` with `skip_reason="livestream_never_ended"`.
...
- `email_verified` — boolean; when `false` the `login` route rejects the attempt with an explicit "verify your email" flash before ever calling `login_user()` (which would also reject it silently via `is_active`, since `is_active = not is_locked and is_verified`). Set to `true` when the user clicks the verification link in their signup email. Key absent defaults to `true` for backward compatibility with existing accounts. Generated during `POST /register` as `false` and verification tokens are created; marked `true` by `GET /verify-email/<token>` after token validation, which is also where the admin's "new user" ntfy notification fires (not at registration — see `_web_ntfy` below). An account left unverified for more than `_UNVERIFIED_USER_MAX_AGE_HOURS` (72h, in TubeNews.py) is deleted by `_cleanup_stale_unverified_users()`.
...
| `_web_ntfy(title, message, priority)` | Sends a best-effort ntfy.sh notification from web events (email verification, manual run trigger, admin user delete/bulk-delete). No-op when `ntfy_topic` is not configured; exceptions are silently swallowed so a notification failure never breaks a request. The "new user" notification fires from `verify_email()`, not `register()` — anyone can POST to `/register` with an address they don't control, so notifying at that point would just relay spam-bot noise. |
...
| GET/POST | `/register` | `register` | Registration form (rate-limited: 5/hour). Includes a hidden honeypot field (`website`); a filled value pretends success without creating an account, writing to disk, or sending email. |
...
| POST | `/admin/users/bulk-delete` | `admin_users_bulk_delete` | Delete several accounts at once (form field `uids`, one per checkbox) — e.g. clearing out a batch of spam/bot registrations. No per-account email confirmation (only the browser-side `confirm()` before submit); silently skips the acting admin's own account if selected. |
...
| POST | `/admin/feeds/<channel_id>/delete` | `admin_feed_delete` | Remove a channel from config; calls `_wsb_unsubscribe` after saving when WebSub is configured. Answers JSON when the request carries `X-Requested-With: XMLHttpRequest` so the page can drop the row in place; a plain form POST still redirects. |
| POST | `/admin/feeds/<channel_id>/toggle` | `admin_feed_toggle` | Enable/disable a channel, subscribing or unsubscribing from WebSub to match. Answers JSON (`{ok, disabled, channel_name}`) for an `X-Requested-With: XMLHttpRequest` request so the row updates without a reload; a plain form POST still redirects. Do **not** fetch this with `redirect: "manual"` — a redirect returns an opaque response with `status === 0`, indistinguishable from a failure. |
...
- **Files to maintain:**
  ...
  - `DEVREF.md` — full developer reference (function signatures, data formats, design decisions)
