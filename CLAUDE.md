...
- **WebSub daemon (default mode).** YouTube pushes new video notifications; processor thread wakes every ~1 min and works through `state/queue/push_queue.json`. Use `--single-run` for cron-style use. **The receiver thread must start before any subscribe POST** — the hub confirms a subscription by calling `hub.callback` back with `hub.challenge`, so subscribing while the listener is down means nothing can answer and every request waits out its timeout. `_wait_for_receiver()` gates this. **The hub is shared and rate-limits**: it answers 429/503 with a `Retry-After` and can take ~20s to do so. `_wsb_note_hub_throttle()` records that hub-wide and `_wsb_subscribe`/`_wsb_unsubscribe` skip until it clears — walking a list of 22 channels after being throttled only deepens it. `websub_post_timeout` defaults to 30s for the same reason: at 10s the client hung up before the 503 arrived, so throttling looked like a network hang.
...
- **Transcript caching.** `transcript.txt` existence skips Supadata. Delete it to re-fetch. A cached transcript costs nothing, so running out of Supadata budget must never stop a cached video from reaching the Gemini phase — the quota paths `continue`, never `break`, or one unfetchable video starves every cached one behind it in the queue.
